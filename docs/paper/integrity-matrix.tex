...
% Draft extracted from the repository evidence at tag v0.5.0.
...
%       admission, Table 1, S6; "four-cell" appears only as history
...
\date{Draft --- extracted from repository state \texttt{v0.5.0}, 2026-08-07}
...
unchanged) --- all reported in the limitations. Ground truth is
...
(1)~A controlled comparison of six boundary configurations --- decoding
enforcement crossed with boundary repair policy, a fault injector in the
unconstrained column and (pre-registered) downstream of the active
schema, and a shared control arm --- with ground truth read
from a persisted external system of record. The design is not a complete
factorial (\S\ref{sec:design}). State-grounded scoring itself is
...
schema configuration under \emph{natural} conditions only; within the
original four cells, no configuration subjected active decoding
enforcement to a forced violation --- the pre-registered follow-up
cells E/F later did exactly that (fault injected \emph{downstream} of
the active schema; results in \S\ref{sec:limits}). The findings are
worded accordingly (F3 claims invisibility of the
...
these runs cached tokens were zero --- verified by recomputing all 203
...
million as recorded; the Postgres and full-schema arms on the baseline
model's rates)
...
the run and is reported as a verification. The condition is positional,
indifferent to what sits upstream of the absorption and to how strong
the downstream check is.
...
construction. That is the entire price of knowing.
...
alternative to refusal was not recovery; it was corruption. The design
is \emph{still} not a complete factorial: the
...
requirement rests on the three demonstrated instances: cell \cellA{},
cell E, and cell E re-run under full-schema boundary validation
...
\paragraph{Data availability.} Repository tag \texttt{v0.5.0}:
implementation, restriction-enforcing test suite (27 passed, 7
...
audit trails), the full-schema boundary arm's own snapshots
