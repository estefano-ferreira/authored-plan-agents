...
%   - Verified by direct fetch, 2026-08-06: arXiv 2606.14589 (Wei Wu,
%       silent-failure taxonomy) -- abstract confirms gray-failure
%       framing and the error-swallowing class; cited without verbatim
%       quotes because only the abstract was read.
...
corrupted records. We reproduce this failure as a controlled four-cell
comparison --- \emph{decoding constraint} (provider-enforced response
schema vs.\ schema stripped) crossed with \emph{boundary policy}
(tolerant repair vs.\ a strict retry-once-then-fail output-contract
guard), a fault injector forcing format violations in the unconstrained
cells --- on one codebase and one model, with ground truth read directly
from the persisted SQLite of an external system of record. Two findings
...
corruption. Second, the guard's price sheet: under enforced decoding
...
zero. Latency means are omitted: they are contaminated by free-tier
...
\toprule
 ...
 & (no schema, tolerant) & (no schema, strict) \\
\midrule
\rowcolor{gray!12}
\multicolumn{3}{@{}l@{}}{\itshape what the platform reports} \\
Agent report & \statuscompleted{} 10/10 & \statusfailedclean{} 10/10 \\
Contract counters (viol.\,/\,retry\,/\,fail) & 0 / 0 / 0 & 10 / 10 / 10 \\
Repair count & 10 & 0 \\
\midrule
\rowcolor{gray!12}
\multicolumn{3}{@{}l@{}}{\itshape what the database holds} \\
Rows in ERP & 10 & 0 \\
Valid rows & \cellcolor{gray!25}\textbf{0} & --- \\
\bottomrule
\end{tabular}
\caption{What each observer sees, cells \cellA{} and \cellB{} (data from
Table~\ref{tab:matrix}; no new measurement). The separation between the
two bands is the finding: in cell \cellA{}, three platform-side
instruments agree the runs succeeded --- status \statuscompleted{},
contract counters at zero, ten ``successful'' repairs --- while the
business database holds ten corrupted records and zero valid ones.}
\label{fig:observers}
\end{figure}
...
ten successes, ten corrupted rows in the business database.
...
\paragraph{Not a complete factorial.} The fault arm exists only in the
no-schema column (\S\ref{sec:design}); the two missing cells ---
schema~+~injected fault, under each boundary policy --- are the only ones
that would show the boundary operating \emph{with} decoding enforcement
active, and they are mechanically non-trivial to predict: the injector
corrupts \emph{downstream} of the provider, so under an enforced schema
the injected fence wraps schema-valid JSON --- tolerant repair would
plausibly recover \emph{valid} content there, while the strict guard
would refuse a recoverable corruption. Either outcome would sharpen the
guard's price sheet, which is exactly why these cells are queued behind a
pre-registration (this repository's discipline for any new number) rather
than run ad hoc.
...
described in gray-failure vocabulary, whose error-swallowing class the
absorption step measured here instantiates~\cite{wu2026}; the
...
report, per-cell SQLite snapshots \texttt{matrix-\{A,B,C,D,control\}.sqlite},
