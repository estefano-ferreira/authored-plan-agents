...
% Draft extracted from the repository evidence at tag v0.4.0.
...
\date{Draft --- extracted from repository state \texttt{v0.4.1}, 2026-08-07}
...
system-of-record engine (reported in the limitations). Ground truth is
...
level (the provider-side \texttt{responseSchema}); the boundary itself
applies a strict decode-shape check --- \texttt{json.loads}, a
bare-JSON-object test, and presence of the contract's required keys,
with no type, enum, format, or \texttt{additionalProperties}
validation --- to the content that feeds a business write, at a point
the generator cannot bypass; on violation, at most one retry, then a
typed failure --- nothing unvalidated persists. Consequently, the
paper's boundary result is about the \emph{position} of this check
relative to absorption, not about the strength of its validation. This
...
these runs cached tokens were zero --- verified by recomputing all 123
...
million as recorded; the Postgres arm on the baseline model's rates)
...
model held fixed) and reproduced both readings exactly --- the results
are engine-portable and a SQLite artifact is ruled out --- but the
circularity itself is untouched: the engine varied, the owner did not.
...
evidential support. First, an output-contract guard --- a decode-shape
check at the boundary --- on the business write path is an
architectural requirement, not an
optimization: its idle cost was measured at zero in the tested
configuration (a bounded observation, F3/F4, \S\ref{sec:limits}) and its
...
--- this requirement rests on the measured price sheet alone. Second,
...
requirement rests on the two demonstrated instances, cells \cellA{} and
E. Third, repair-before-check must be made structurally impossible on
...
on Proposition~1's structural argument, of which cells \cellA{} and E
are observed instances rather than its proof. The reference
...
\paragraph{Data availability.} Repository tag \texttt{v0.4.1}:
