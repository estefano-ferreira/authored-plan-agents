...
optimization: its measured idle cost is zero and its per-violation price
is one model call, so omitting it buys nothing and forfeits the only
loud-failure path this study observed. Second, platform-side telemetry
...
{AgentTelemetry}: A fault detection benchmark and toolkit for {LLM}
agent observability.
