...
% Draft extracted from the repository evidence at tag v0.3.0.
...
\date{Draft --- extracted from repository state \texttt{v0.3.0}, 2026-08-06}
...
one model (a pre-registered cross-model arm, reported separately in the
limitations, reruns two cells in a second model family), with ground
...
these runs cached tokens were zero --- verified by recomputing all 83
recorded per-execution costs (the six-cell comparison, the control arm,
and the cross-model arm at its own list rates: \texttt{gpt-4o-mini},
\$0.15 / \$0.60 per million) from their token counts alone, which
...
\paragraph{Data availability.} Repository tag \texttt{v0.3.0}:
